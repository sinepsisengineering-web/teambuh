...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView }) => {
    ...
    const superAdminMode = typeof window !== 'undefined' && localStorage.getItem('superadmin_mode') === '1';
    const superAdminTenant = typeof window !== 'undefined' ? localStorage.getItem('superadmin_tenant') : '';
    ...
    return (
        <aside className="w-64 bg-[linear-gradient(135deg,#1E1E3F_0%,#312e81_50%,#1E1E3F_100%)] text-white p-4 flex flex-col flex-shrink-0 h-screen">
            ...
            <div className="mb-8 px-2">
                ...
                {superAdminMode && (
                    <div className="mt-2 rounded-md bg-amber-400/20 border border-amber-300/40 px-2 py-1">
                        <p className="text-[11px] font-semibold text-amber-200">Вы в режиме SuperAdmin</p>
                        {superAdminTenant && (
                            <p className="text-[11px] text-amber-100/90 truncate">{superAdminTenant}</p>
                        )}
                    </div>
                )}
                {user && (
            ...
            <nav className="flex flex-col gap-1 flex-1">
                ...
                {/* ВРЕМЕННО СКРЫТО: старый календарь
                ...
                {superAdminMode && (
                    <button
                        onClick={() => {
                            ...
                            localStorage.removeItem('superadmin_tenant');
                            window.location.href = 'https://admin-oleg.teambuh.ru';
                        }}
                        className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors text-amber-300 hover:bg-amber-500/20 hover:text-amber-200"
                    >
                        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        ...
                        <span className="font-medium">Выйти из SuperAdmin</span>
                    </button>
                )}
